...
exports.handler = async (event) => {
  ...
  // 2. Alle Symbole – kein Pre-Market nötig, Gap simulieren falls Markt offen
  const isPreMarket = validQuotes.some(q => q.isPreMarket);
  const relevant = validQuotes.map(q => {
    // Kein Pre-Market? Kleinen Dummy-Gap setzen damit Score funktioniert
    if (!q.isPreMarket && Math.abs(q.chgPct) < 0.05) {
      return { ...q, chgPct: 0.3, simulatedGap: true };
    }
    return q;
  }).filter(q => {
